...
import { db } from "./db";
import { eq, gte, lt, and, desc, asc, sql } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  async updateReservation(id: number, data: UpdateReservation): Promise<Reservation | undefined> {
    ...
    // Solo se permiten modificaciones para reservas que no han pasado
    const currentDate = new Date();
    if (new Date(reservation.startDate) <= currentDate) {
      return undefined; // No se puede modificar una reserva que ya pasó
    }
  ...
  async getReservationsByYear(year: string): Promise<Reservation[]> {
    ...
    return await db
      ...
      .where(
        and(
          gte(reservations.startDate, startOfYear),
          lt(reservations.startDate, endOfYear)
        )
      );
  }
    ...
    return await db
      ...
      .where(
        and(
          gte(reservations.startDate, startOfYear),
          lt(reservations.startDate, endOfYear),
          sql`${reservations.status} in ('pending', 'modified')`
        )
      );
  }
    ...
    return await db
      ...
      .where(
        and(
          gte(reservations.startDate, startOfYear),
          lt(reservations.startDate, endOfYear),
          sql`${reservations.status} in ('approved', 'rejected', 'cancelled')`
        )
      )
  ...
  async getReservationStatsByYear(year: string): Promise<ReservationStats> {
    ...
    // Crear un mapa de IDs de usuario a nombres de usuario
...
// Cambiamos a usar la implementación de base de datos
